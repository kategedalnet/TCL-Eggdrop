...
# system stuff
variable quizbasedir        scripts/azza/quiz/quiz/
...
set quizconf(quizchannel)        "#kotamobagu"
...
bind pub m !stop tmcquiz_stops
bind pub m !exit tmcquiz_exit
bind msg m !userquest tmcquiz_userquest
...
set quizshorthelp [list \
	"© by ktg@DALnet" \
	"Most important commands are: !start, !rank, !score" \
...
proc tmcquiz_stop {handle idx arg} {
    ...
    mx_log "--- Game stopped."
    mxirc_say $quizconf(quizchannel) "Puzzle Game stopped."
    return 1
}
...
proc tmcquiz_halt {handle idx arg} {
    ...
    mx_log "--- Game halted."
    mxirc_say $quizconf(quizchannel) "0,1Puzzle Quiz halted.  Say 9!start 0,1for new questions."
    return 1
}
...
#	    }
	    ...
	    mxirc_say $channel "0,14 $answer 0,1 answered by 0,14 $nick 0,1 after 4 $duration  0,1score 4 +$theq(Score) pts 0,1 and now has 4 $userarray(score) 0,1 points"



	    ## honor good games!
	    if {$lastsolvercount == 3} {
		mxirc_say $channel "0,1Three in a row! extra point 9 8 pts "
                tmcquiz_rank_set $botnick 0 "$nick +8"
	    } elseif {$lastsolvercount == 6} {
		mxirc_say $channel "0,1Five in a row! You receive extra point 9 14 pts "
		tmcquiz_rank_set $botnick 0 "$nick +14"
	    } elseif {$lastsolvercount == 10} {
		mxirc_say $channel "0,1Ten in a row! This is really rare, so you receive 9 22 pts "
		mx_log "--- $nick has ten in a row.  score += 22"
		tmcquiz_rank_set $botnick 0 "$nick +22"
	    } elseif {$lastsolvercount == 15} {
		mxirc_say $channel "0,1Fivety in a row!, so you receive 9 30 pts "
		tmcquiz_rank_set $botnick 0 "$nick +30"
	    } elseif {$lastsolvercount == 20} {
		mxirc_say $channel "0,1Twenty in a row! This is really rare, so you receive 9 40 pts "
...
#			set waitforrank 15
	    ...
	    regsub -all "\#(\[^\#\]*\)\#" $theq(Answer) "\\1" answer
	    mxirc_say $channel "0,1$nick solved own question after 12 $duration 0,1 and gets no points, keeping 12 $userarray(score) 0,1 points on rank 7[mx_get_rank_pos $nick]0,1. The answer was: 12 $answer "
...
#        set ans "word : $answer"
	...
	set txt "$txt question number $qnum_thisgame"
	if {$theq(Score) > 1} {
#	    set txt "$txt"
	}
	if {[info exists theq(Author)]} {
	    set txt "$txt by $theq(Author):"
	} else {
	    set txt "Hint: $txt:"
	}
#        set ans "word :[randomanswer $answer]"
#        mxirc_say $channel $ans
	...
	if {[info exists theq(Category)]} {
	    ...
	    set txt "0,1Hint : 0,14$txt$theq(Question)"
	}
      set ans "0,1Word : 0,14[randomanswer $answer]"
...
proc tmcquiz_solve {handle idx arg} {
	...
	if {[mx_str_ieq $botnick $handle]} {
        ...
        regsub -all "\#(\[^\#\]*\)\#" $theq(Answer) "\\1" answer
	set txt "0,1$txt solved after 4 [mx_duration $timeasked]$solver 0,1 The answer is: 4 $answer "
	mxirc_say $quizconf(quizchannel) $txt

	# remove area of tip generation tags
#	regsub -all "\#(\[^\#\]*\)\#" $theq(Answer) "\\1" answer
#	mxirc_say $quizconf(quizchannel) "0,1The answer is: $answer"
...
proc tmcquiz_pub_score {nick host handle channel arg} {
	...
	if {[info exists userlist($target)]} {
		...
		mxirc_say $channel "0,1$nick You have 4 $userarray(score) 0,1points and are on rank 4 $pos  0,1in the current game"
	    } else {
		mxirc_say $channel "0,1$nick did not make any points in this game."
	    }
	} else {
	    mxirc_say $channel "0,1$nick not yet listed for the current game."
	}
...
#    }
	...
	if {($newscore == "") || ($user == "")} {
		...
		set txt "0,1$user have extra score 4 [format "%+d" [expr $newscore - $oldscore]] 0,1 new score now 4 $newscore points 0,1 on rank [mx_get_rank_pos $user]."
#		set prefix
...
$quizconf(maxranklines)."
	...
	lappend lines "0,14Highscore Current Top $length:"
	set pos 1
	set prevscore 0
	foreach u [lsort -command mx_sortrank [array names userlist]] {
	    array set aa $userlist($u)
		...
		set text [format "0,1%2d " $pos]
	    }
	    set text [format "0,1$text %12s :: %5d pts." $u $aa(score)]
	    if {$pos == 1} {
		set text "0,1$text"
	    }
	    lappend lines $text
	...
	lappend lines "0,14Rank started [mx_duration $timerankreset] ago."
    }
...
proc tmcquiz_on_nickchanged {nick host handle channel newnick} {
    global userlist quizconf
 	...
 	if {$oldentry(mask) == [maskhost $host]} {
 	    ...
 	    if {$addscore != 0} {
 		mxirc_notc $newnick "Your old scores of $addscore were collected, since I know your host."
 	    }
 	} else {
 	    set addscore $oldentry(score)
 	    if {$oldentry(score) != 0} {
...
proc tmcquiz_on_joined {nick host handle channel} {
    ...
    set text "Welcome to Puzzle Quiz, © Say \"!start\" to get the current question"
	...
	if {$aa(mask) == [maskhost $host]} {
	    if {$aa(score) > 0} {
		mxirc_notc $nick "You are listed with $aa(score) points on rank [mx_get_rank_pos $nick]."
	    }
	} else {
	    mxirc_notc $nick "You are listed with $aa(score) points on rank [mx_get_rank_pos $nick]."
	}
    ...
    if {$quizstate == "paused"} {
	mxirc_notc $nick "The current game is paused after $qnum_thisgame questions."
    } elseif {$qnum_thisgame != 0} {
	mxirc_notc $nick "There is a game running, $qnum_thisgame questions have already been asked."
    }
...
## Tips To Use
set TipsToUse {
"0,1Type 7!start0,1 - get the current question or start over"
"0,1Type 7!rank0,1 - shows you the current rank list"  
"0,1Type 7!score0,1 - tells your current scores" 
"0,1Type 7!stop0,1 - Stop Puzzle Game"
"0,18Rules0,1 - No excessive flooding and scripting"
"0,18Rules0,1 - Please remove any script-advertisements from you quit and part messages"
"0,18Rules0,1 - No offending, racism, harassments, impersonation, abuse of nicknames or discrimination"
"0,18Rules0,1 - No clones, cheating, exploiting of known weaknesses"
"0,1Type 7!comment <whatever is wrong>0,1 -  the bot will record it"
"0,1Type 7!userquest <question>::<answer>0,1 - in a query"
}
